...
- To submit a query, the querier sends a [query file](QueryLanguage.md) to `Backend`. A backend system can set its asking price to run computation for this query. Each listing will access some specified subset of listings in market.
...
- Each query causes some loss in data privacy. A charge is leveled to account for this cost. This charge may be adaptive and depend on history of past queries by given user (TODO: More details)
